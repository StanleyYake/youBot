...
{
  ros::init (argc, argv, "planning_pipeline_tutorial");
  ...
  pose.pose.position.x = 0.038;
  pose.pose.position.y = 0.006;
  pose.pose.position.z = 0.534;
...
//      std::vector<double> tolerance_angle(3, 0.02);// Position_constraints/constraint_region/premitives/dimention
  ...
  ROS_INFO("Visualizing the Pose Goal trajectory used pipeline.");
...
//  group.move();
  ...
  /* Visualize the joint space goal used pipeline. */
ROS_INFO("Visualizing the joint space goal used pipeline.");
  res.getMessage(response);
  display_trajectory.trajectory_start = response.trajectory_start;
  display_trajectory.trajectory.push_back(response.trajectory);
...
//  group.move();
  ...
  const robot_model::JointModel* joint_model = joint_model_group->getJointModel("arm_joint_5");
  const robot_model::JointModel::Bounds& joint_bounds = joint_model->getVariableBounds();
  std::cout<< "arm_joint_5 min position bounds: "<< joint_bounds[0].min_position_ << std::endl;
  std::cout<< "arm_joint_5 max position bounds: "<< joint_bounds[0].max_position_ << std::endl;
...
//  tmp_values[0] = joint_bounds[0].min_position_ - 0.01;
  ...
  /* Visualize the trajectory */
ROS_INFO("Visualizing change bounds results.");
  res.getMessage(response);
  display_trajectory.trajectory_start = response.trajectory_start;
  display_trajectory.trajectory.push_back(response.trajectory);
...
//  robot_trajectory::RobotTrajectoryPtr result_trajectory = res.trajectory_;

  moveit::planning_interface::MoveGroup::Plan my_plan;
//  my_plan.trajectory_ =  res.trajectory_;// moveit_msgs::RobotTrajectory = robot_trajectory::RobotTrajectoryPtr
