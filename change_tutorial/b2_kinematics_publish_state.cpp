...
{
    ros::init(argc, argv, "youbot_move_group_interface_changed_current", ros::init_options::AnonymousName);
//    ros::init(argc, argv, "constraint_demo");
    ...
    ros::Publisher state_publisher = nh.advertise<moveit_msgs::DisplayRobotState>("display_robot_state", 1);
    ...
    for(int cnt = 0; cnt <5 && ros::ok(); cnt++)
    {
        ...
        robot_state::robotStateToRobotStateMsg(*kinematic_state, state_msg.state);
...
//robot_state::RobotState current_state = planning_scene_ptr->getCurrentState();
//    current_state.update();
//    right_arm.setStartState(current_state);
